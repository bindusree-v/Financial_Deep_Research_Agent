"""
Telecom Agent module for the Financial Research Agent.
This agent is responsible for handling queries specific to the Telecom sector.
"""
...
class TelecomAgent:
    ...
    def __init__(self):
        """
        Initializes the TelecomAgent by setting up its dependencies.
        ...
        print("Telecom Sector Agent initialized")
    ...
    def run(self, query: str) -> str:
        ...
        Args:
            query (str): The user's query about the Telecom sector.
            
        Returns:
            str: The generated markdown report containing the research findings.
        """
        # Notify that the deep research process is beginning
        print("Telecom Agent starting deep research...")
        
        # Run the research loop for the query and specify the "Telecom" sector
        findings = self.research_loop.run(query, "Telecom")
        
        # Generate a final markdown report using the gathered findings
        report = self.report_generator.generate(findings, "Telecom", query)
